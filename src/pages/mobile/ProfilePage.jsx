...
export default function ProfilePage({ user, onNavigate }) {
  const [stats, setStats] = useState({ listings: 0, trades: 0, rating: 0, coins: 0 });
  ...
  const loadStats = async () => {
    try {
      const [txns, ach, reviews] = await Promise.all([
        Transaction.filter({ user_email: user.email }),
        UserAchievement.filter({ user_email: user.email }),
  ...
  return (
    <div style={styles.page}>
      ...
      <div style={styles.topBar}>
        ...
        <button style={styles.settingsBtn} onClick={handleSignOut}>Sign Out</button>
      </div>
      ...
      <div style={styles.profileCard}>
        ...
        <div style={styles.statsRow}>
          <div style={styles.stat}><span style={styles.statNum}>{stats.listings}</span><span style={styles.statLabel}>Listings</span></div>
      ...
      <div style={styles.walletCard}>
        <div style={styles.walletTop}>
          ...
          <button style={styles.cashoutBtn} onClick={() => onNavigate('cashout')}>Cashout</button>
        </div>
...
const styles = {
  ...
  settingsBtn: { background: 'transparent', border: '1px solid #333', color: '#888', padding: '6px 12px', borderRadius: 8, cursor: 'pointer', fontSize: 13 },
